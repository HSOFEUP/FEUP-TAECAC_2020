...
df_subset_Ent            <- df_subset[, setdiff( names( df_subset),c( "Longitude",
                                                                 "Latitude",
                                                                 "First_Road_Number",
                                                                 "Second_Road_Number",
...
#####Analysis: For the entropy analysis, this are the most relevant variables regarding the accident_Severety. Later we do analysis in those suggested variables and compare with out initial guess.
*****

#### 2.1.1 For our initial guess, lets do a analisys with variables we find relevant.
```{r}
...
#### 2.3.2 Focus in the Light_Conditions=Darkness - lights lit
```{r}
...
##### 2.3.3 - Focus in the Road_Surface_Conditions=Wet or damp
```{r}
...
##### 2.3.4 - Focus in the Day_Period=Night
```{r}
...
##### Analisys: As we can analyse in the graph, is possible to identify to major groups of itemsets that (Co-occur frequently), namelly the  darkness condition and rural areas

##### Analisys: Also is possible to identify the (Co-occur frequently) of Surface conditions Dry, and not and junction within 20 meters.
...
##### Analysis: A quick analisys shows that Accident_Severity=Fatal, Light_Conditions=Darkness - lights lit, Road_Type=Roundabout, Junction_Detail=Roundabout have an reasonable lift suggesting that they Co-occurr together, what makes sense regarding the fact that suggest a city crossing, (City))
...
##### Analysis: We may find some rules that area obvious to us such as Speed_limit=60, Month=Nov, Day_Period=Evening} => {Light_Conditions=Darkness - no lighting}. In order to obtain more relevant ones we gona focus in the cases that acciddents are fatal.
...
##### Analysis: We may find that {Accident_Severity=Fatal, Speed_limit=60, Light_Conditions=Daylight} => {Urban_or_Rural_Area=Rural} Co-occur together the lift is not very high, but is bigger than one what may suggest that they are positive co-related.
...
##### Analysis: From an quick analisys, Speed_limit=60, Month=Nov, Day_Period=Evening} => {Light_Conditions=Darkness - no lighting} is a coerent rule but not very usefull.
...
##### Analysis: We may find the rule {Light_Conditions=Darkness - no lighting,  Road_Type=Dual carriageway, Casualties_Class=Medium} => {Speed_limit=70} contains a co-occurence of high number of car involved. Clearely a auto -way.
...
##### Analysis: Of course the most commun ones appears first with strong conviction. However we are insterested in a more grain set of groups of itemsets and rules, namelly those that contain Fatal accidents as consequent.
...
##### Analysis: The diagram may suggest that the fatal accidents happens with no particular positive correlations (lift =1 -> Independent).
...
##### Analisys: Clearely an rule regarding a high speed motorway
...
##### 4.1.1 - Adding a extra measure of quality
```{r}
...
##### Analysis: Police always attend fatal accidents, normal

#### 4.1.2 - Generate rules with rhs as fatal
```{r}
## Generate the rules with consequent as Accident_Severity=Fatal"
rules_subset <- apriori(trans_sub, parameter = list(supp=min_supp, conf=min_conf,
                                                    target ="rules"),
...
#### Analysis: We see that the almost the same results were attained.

#### Analysis: Police always attend fata crashes

#### Analysis: {Junction_Detail=Not at junction or within 20 metres} => {Accident_Severity=Fatal} Ok no infraestructure or juctions.
